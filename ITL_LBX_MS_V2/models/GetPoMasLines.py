...
class GetPoLines(models.Model):
    ...
    header_table = fields.Many2one(
        "get_po_mas", string="Purchase Order Reference", ondelete="cascade"
    )

    # Fields from PurchaseOrder
    po_number = fields.Char(string="PO Number")
    StatusField = fields.Selection(
        [
            ("Success", "Success"),
            ("Open", "Open"),
            ("Cancelled", "Cancelled"),
    ...
    # Add the new field
    line_number = fields.Char(string="No", default=1, store=True, readonly=True)

    @api.model
    def create(self, values):
        # Override create method to set the line_number when creating a new record
        lines = self.search(
            [("header_table", "=", values.get("header_table"))]
        )  # Assuming PurchaseorderNum is a reference field to link lines of the same set
    ...
    care_instruction_set_code = fields.Char(
        string="Care Intruction Set Code",
        related="header_table.care_instruction_set_code_2",
        store=True,
    )

    POwithLine = fields.Char(
        string="Po With Lines", compute="_compute_po_line", store=True
    )
    ...
    def _compute_po_line(self):
        for record in self:
            if record.po_number and record.purchase_order_item:
                ...
                record.POwithLine = (
                    f"{record.po_number}-{PurchaseOrderItem_without_zeros}"
                )
            else:
    ...
    def _compute_so_ref_lv(self):
        for record in self:
            ...
            if record.sales_order_line and record.sales_order_item:
                ...
                record.SoRefLv = (
                    f"{record.sales_order_line}/{so_order_item_without_zeros}"
                )
            else:
                record.SoRefLv = False

    size_range = fields.Char(
        string="Size Range", related="header_table.Size_Range_name", store=True
    )
    size_lv = fields.Char(string="Lv Size")
    size_mapping = fields.Many2one(
        "size_mapping_lines",
        string="Size Mapping",
        compute="_compute_size_mapping",
        store=True,
    )
    ...
    def _compute_size_mapping(self):
        for record in self:
            size_mapping = self.env["size_mapping_lines"].search(
                [
    ...
    ChoosePo = fields.Selection(
        string="Choose Po", related="header_table.ChoosePo", store=True
    )
    DelDate = fields.Date(
        string="Delivery Date", related="header_table.DelDate", store=True
    )
    vsd = fields.Char(string="Vsd", related="header_table.vsd", store=True)
    vendor_id = fields.Char(
        string="Vendor Id", related="header_table.vendor_id", store=True
    )
    Status = fields.Selection(
        string="Status", related="header_table.Status", store=True
    )
    RnNumber = fields.Selection(
        string="Rn Number", related="header_table.RnNumber", store=True
    )
    CaNumber = fields.Selection(
        string="Ca number", related="header_table.CaNumber", store=True
    )
    ChainID = fields.Many2one(
        tring="Chain Id", related="header_table.ChainID", store=True
    )
    ...
    FactoryID = fields.Char(
        string="Factory Id", related="header_table.FactoryID", store=True
    )
    ProductRef = fields.Char(
        string="Product Ref", related="header_table.ProductRef_name", store=True
    )
    Collection = fields.Char(
        string="Collection", related="header_table.Collection_name", store=True
    )
    silhouette = fields.Char(
        string="Silhouette", related="header_table.silhouette_name", store=True
    )
    ItlCode = fields.Char(
        string="ItlCode", related="header_table.ItlCode_name", store=True
    )
    Coo = fields.Char(string="Coo", related="header_table.Coo_name", store=True)
    season = fields.Char(
        string="Season", related="header_table.season_name", store=True
    )
    ChainID_id = fields.Char(
        string="Chain Id", related="header_table.ChainID_id", store=True
    )
    ChainID_id_name = fields.Char(
        string="Chain Id Name", related="header_table.ChainID_id_name", store=True
    )
    date_of_manufacture = fields.Date(
        string="Date Manufacture",
        related="header_table.date_of_manufacture",
        store=True,
    )
    date_of_manufacture_last_four_letters = fields.Char(
        string="Date Manufacture",
        compute="_compute_date_of_manufacture_last_four_letters",
        store=True,
    )
    Customer_name = fields.Char(
        string="Customer Name", related="header_table.Customer_name", store=True
    )
    ...
    additional_care_instruction = fields.Char(
        string="Additional Care Instruction",
        related="header_table.additional_care_instruction_name",
        store=True,
    )
...
class GetPoLinesMainLable(models.Model):
    ...
    purchase_order_version = fields.Char(string="PO Version")
    cid = fields.Char(string="Composition ID")
    po_date = fields.Date(string="PO Date")
    last_revision_date = fields.Date(string="Last Revision Date")
    currency = fields.Char(string="Currency")
    consignee_name = fields.Char(string="Consignee Name")
